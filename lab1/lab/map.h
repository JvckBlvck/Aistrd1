...
template<class T>
Iterator<T> * map<T>::createBfsIterator() {
...
template<class T>
inline map<T>::node::node(size_t key, T data)
{
	this->key = key;
	this->data = data;
	color = 'r';
...
template<class T>
inline void map<T>::finding(int * check, T * finded, size_t key)
{
	node*cur = root;
...
template<class T>
inline list<size_t> map<T>::get_keys()
{
...
template<class T>
inline list<T> map<T>::get_values()
{
	list<T> list_data;
	for (Iterator<T> *tmp = createBfsIterator(); tmp->hasNext(); tmp->next())
	{
